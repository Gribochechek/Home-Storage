...
import dialogWindows.PartEditWindow;
...
public class ListenerForButton implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()== Main.mainWindow.btnEditPart) {
			
			
			try {
				int idOfeditingPart = (int) Main.mainWindow.automotivePartsTable
						.getValueAt(Main.mainWindow.automotivePartsTable.getSelectedRow(), 0);
				PartEditWindow dialog= new PartEditWindow(idOfeditingPart);
				dialog.setVisible(true);
				dialog.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
			} catch (Exception e1) {
